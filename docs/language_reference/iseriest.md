...
`ISeries<T>` is an interface that is implemented by all NinjaScript classes that manage historical data as an `ISeries<double>` (Open, High, Low, Close, etc), used for indicator input, and other object data.  Please see the help guide article on [Working with Price Series](../ninjascript/working_with_price_series.md) for a basic overview on how to access this information.
