...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](discrepancies_real-time_vs_bac.htm) >>  **Navigation:**  [Operations](operations.htm) > [Strategy Analyzer](strategy_analyzer.htm) >  Discrepancies: Real-Time vs Backtest | [Previous page](2d__3d_optimization_graphs.htm) [Return to chapter overview](strategy_analyzer.htm) [Next page](saving_strategy_parameter_temp.htm) |

You should expect that a strategy running real-time (live [brokerage account](connecting.htm), live [market simulation](simulation.htm), [Playback connection](playback_connection.htm) etc...) will produce different results than the performance results generated during a backtest. This difference may be more easily seen on certain Bars types (e.g. Point and Figure or Renko) than others due to their inherent nature in bar formation.
...
•During real-time operation, you have a choice to run a strategy tick by tick ([Calculate](running_a_ninjascript_strategy.htm) set to 'On Each Tick') which can produce different results. This is because you can have a signal that executes an order at the close of a bar but when running tick by tick, while in a bar a signal condition can be true although its false at the close of the same bar.

Differences in chart data
-------------------------

•If you run a strategy in real-time on DAY1 and then DAY2, you are now backtesting your strategy on DAY1 data instead of processing like it did in real-time so there could be differences. You should understand how [chart bars are built](how_bars_are_built.htm).
