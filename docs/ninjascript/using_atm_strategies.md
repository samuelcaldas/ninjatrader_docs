...
You can create an automated strategy that generates a trade signal that executes a NinjaTrader [ATM Strategy](../strategies/atm_strategy.md).
...
- Strategy set up parameters such as [EntriesPerDirection](../strategies/entriesperdirection.md), [EntryHandling](../strategies/entryhandling.md), [IsExitOnSessionCloseStrategy](../strategies/isexitonsessionclosestrategy.md) do not apply when calling the [AtmStrategyCreate()](../strategies/atmstrategycreate.md) method
...
- ATM Strategy stop orders can either be StopMarket or StopLimit orders, depending on which type is defined in the ATM Strategy Template ([Advanced Options](../strategies/advanced_options.md)[)](../strategies/advanced_options.md) you call in the [AtmStrategyCreate()](../strategies/atmstrategycreate.md) method in your NinjaScript strategy. To make the distinction clear which is used, following a naming convention for the template name is highly suggested (i.e. AtmStrategyTemplate\_STPLMT)
...
- If the order entry window [ATM Strategy Selection Mode](../strategies/atm_strategy_selection_mode.md) is NOT in "DisplaySelectedATMStrategyOnly" click on the "CLOSE" button via your middle mouse button (scroll wheel)
