...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\set_up10.htm) >>  **Navigation:**  [NinjaScript](ninjascript-1.htm) > [Educational Resources](educational_resources-1.htm) > [Developing Strategies](developing_strategies-1.htm) > [Beginner - Simple MA Cross Over](beginner_-_simple_ma_cross_ove-1.htm) >  Set Up | [Previous page](beginner_-_simple_ma_cross_ove-1.htm) [Return to chapter overview](beginner_-_simple_ma_cross_ove-1.htm) [Next page](creating_the_strategy_via_the_-1.htm) |

The first step in creating a custom strategy is to use the custom [Strategy Builder](strategy_builder-1.htm). The builder provides two options:
...
The next page will allow you to set defaults for basic properties related to your strategy, including it's [Calculate](calculate-1.htm) and [EntryHandling](entryhandling-1.htm) settings. Click the More Properties button to expose additional properties. For this tutorial, we will not change any basic properties' defaults, and instead will leave them all set to the values shown below:
...
The next page will allow you to configure one or more additional [Bars](bars-1.htm) objects for use by the strategy. For our purposes, we will leave this page blank and move forward by clicking the Next > button.
...
If you want to proceed with this tutorial through [self programming continue here](creating_the_strategy_via_self-1.htm) after pressing the "Unlock Code" button.

If you want to proceed with this tutorial through [using the Strategy Builder please click here](creating_the_strategy_via_the_-1.htm).