...
```csharp
Absolutely, NinjaTrader's Strategy Selection Modes allow you to limit the display in the SuperDOM so that you can run concurrent ATM Strategies.
One of the great features of NinjaTrader is its ability to manage multiple virtual positions in the same market. For example, this allows you to manage a long and short position in the same market simultaneously.
...
For more information please see the [ATM Strategy Selection Mode](atm_strategy_selection_mode.md) section of the user help guide, or attend one of our [free live training events](http://www.ninjatrader.com/webnew/trading_online_events.md).
```
![tog_minus](../images/tog_minus.gif)        Can I have my Auto Trail loosen as I gain ticks in profit?
...
![tog_minus](../images/tog_minus.gif)        Why don't the following ATM Strategy parameters work?
...
![tog_minus](../images/tog_minus.gif)        Can I use the Auto Breakeven and Auto Trail strategies together?
...
![tog_minus](../images/tog_minus.gif)        How do I add an ATM Strategy to an open position?

> If you have opened a position which is currently unprotected by an ATM Strategy, you can easily add an pre-defined ATM Strategy to that position from the [Positions tab](../operations/positions_tab.md) by right clicking on the instrument row, selecting Apply ATM Strategy and selecting the desired pre-defined [ATM Strategy Template](manage_atm_strategy_templates.md) from this sub-menu.    ATM_40

![tog_minus](../images/tog_minus.gif)        Can I manually bracket a position without using an ATM Strategy?
...
![tog_minus](../images/tog_minus.gif)        How do I make one target a "runner" so that it has a Stop Loss only and no Profit Target?

> If you want a target in your ATM Strategy to have a Stop Loss only then set the Profit Target to zero.

![tog_minus](../images/tog_minus.gif)        What happens as one of my ATM orders are rejected?
