...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\submitting_orders_basic_entry.htm) >>  **Navigation:**  [Operations](operations-1.htm) > [Order Entry](order_entry-1.htm) > [Basic Entry](basic_entry-1.htm) >  Submitting Orders | [Previous page](display_overview_basic_entry-1.htm) [Return to chapter overview](basic_entry-1.htm) [Next page](modifying_and_cancelling_orders_basic_entry-1.htm) |

Submitting orders within the Basic Entry order entry window is both easy and efficient.  In addition to entry and exit orders the Basic Entry window also offers access to NinjaTrader's ATM Strategies.  For more information on ATM Strategies please see the [ATM section](advanced_trade_management_atm-1.htm) of the user help guide or attend one of our [free live training events](https://ninjatrader.com/futures/livestreams).
...
|  |
| --- |
| To submit an Order 1.Set the order "Quantity" field ([info](quantity_selector-1.htm))  2.Set the "TIF" (Time in Force) field ([info](tif_selector-1.htm))  3.Set the "ATM Strategy" ([info](atm_strategy_parameters-1.htm))  4.Enter an order with any of the methods described below  BasicEntry_13 |
