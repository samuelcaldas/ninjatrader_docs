...
The "PnL" column calculated values incorrectly with the "Ticks" Calculation Mode when scaling into a position
...
An exception was thrown when dragging a data series after copying it
...
The Trend Channel did not plot parallel lines when moving in some situations
...
Draw.HorizontalLine() could not be used with Equidistant Bar Spacing disabled
...
^TICK historical data values all plotted above 0 on TD Ameritrade connections
...
After an order rejection, Tick Sizes or Point Values could not be changed, even when disconnected
...
CFD's could not be backtested or optimized when the "Include Commissions" property was enabled
...
NinjaScript strategies submitted orders when the Playback slider was updating to a future time
...
Charts sometimes froze after copying and pasting or dragging and dropping chart objects
...
The UI could lock up when using the database rollover feature
...
Plot widths with a value of 1 were the same as those with a value of 2, and were semi-transparent
...
Incorrect list of available connection providers appeared with Simulated (@SIM) license keys
...
Cross conditions triggered alerts even if a Line cross did not occur within the look back period