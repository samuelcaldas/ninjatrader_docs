...
You can optionally reconfigure these default settings to allow for the left click to submit MIT orders from the [SuperDOM Properties](properties_superdom.htm):
...
Or you configure the Middle mouse button to submit stop-market orders from the [SuperDOM Properties](properties_superdom.htm):
...
|  |
| --- |
| Note: It is highly recommended that you review the [Advanced Trade Management (ATM)](advanced_trade_management_atm.htm) section for a complete understanding of order submission and subsequent actions that you can have NinjaTrader automate. |

![tog_minus](tog_minus.gif)

|  |
| --- |
| How to Select an Instrument There are multiple ways to select an Instrument in the SuperDOM window.    •Select the Instrument Selector to open a list of recently used instruments or instruments contained in a predefined list    •With the SuperDOM window selected begin typing the instrument symbol directly on the keyboard. Typing will trigger the Overlay Instrument Selector.    For more Information on instrument selection and management please see [Instruments](instruments.htm) section of the Help Guide.   How to Select an Account A list of all connected accounts will be listed in the Account Selector drop down list. To change the account select the account you wish to trade through via this drop down list. |

![tog_minus](tog_minus.gif)        Understanding order settings

|  |
| --- |
| To submit an Order 1.Set the order "Quantity" field ([info](quantity_selector.htm))  2.Set the "TIF" (Time in Force) field ([info](tif_selector.htm))  3.Set the "ATM Strategy" option ([info](atm_strategy_parameters.htm))  4.Enter an order with any of the methods described below |

![tog_minus](tog_minus.gif)        How to submit a limit order
...
![tog_minus](tog_minus.gif)        How to submit a stop-market order
...
![tog_minus](tog_minus.gif)        Understanding the OCO order (one cancels other) function
