...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\creating_a_backup_archive.htm) >>  **Navigation:**  [Operations](operations-1.htm) > [Backup & Restore](backup__restore-1.htm) >  Creating a Backup Archive | [Previous page](backup__restore-1.htm) [Return to chapter overview](backup__restore-1.htm) [Next page](restoring_a_backup_archive-1.htm) |
...
| Database (Historical trade data) | Contains your historical trade execution data which is used to build reports in the [Account Performance](trade_performance-1.htm) window |
| Historical chart data\* | Chart data which has been recorded from a live connection, [downloaded](download-1.htm) from a data provider, or [manually imported](importing-1.htm) |
