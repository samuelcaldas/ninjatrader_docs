...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\onstatechange.htm) >>  **Navigation:**  [NinjaScript](ninjascript-1.htm) > [Language Reference](language_reference_wip-1.htm) > [Common](common-1.htm) >  OnStateChange() | [Previous page](marketdeptheventargs-1.htm) [Return to chapter overview](common-1.htm) [Next page](setstate-1.htm) |

Definition
----------

An event driven method which is called whenever the script enters a new [State](state-1.htm). The OnStateChange() method can be used to configure script properties, create one-time behavior when going from historical to real-time, as well as manage clean up resources on termination.
