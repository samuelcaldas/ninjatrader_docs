...
![Shared_Trade_Controls_1](../images/shared_trade_controls_1.png)
...
Tip:  If you are trading using multiple ATM Strategies, it is possible to reconfigure the position display to only display the position of the current selected ATM Strategy.  Please see our Help Guide section on [ATM Strategy Selection](../strategies/atm_strategy_selection_mode.md) mode for more information.

> Position Information The current number of contracts in position will be displayed as a number in the position display.  The direction of the position will be also represented in a highly visual manner:
...
| --- | --- |
| Average Entry Price  When in a position, the Average Entry Price Display will show you the current price which is being used to calculate your open PnL.  As you scale in and scale out of position at different prices, your Average Entry Price will be recalculated to reflect the new average price.  The way this is calculated is set under the [Trading](../getting_started/options_trading.md) category of NinjaTrader's general options menu.    |  

> **Note:** The SuperDOM's average entry price will be displayed directly on the [Price ladder display](../strategies/price_ladder_display.md) rather than a text field.
...
> **Tip:** Show realized PnL when Flat will show you your overall account PnL and not the selected instrument's PnL.  If you'd like to see each individual instruments PnL, you can use the Market Analyzer's Realized profit loss and Unrealized profit loss [Columns](../operations/working_with_columns.md) to display this information per instrument.
