...
The ChartBars class provides GUI access related methods and properties to the primary bars series configured on the Chart through the [Data Series](../operations/working_with_price_data.md) menu.   For data access information related to the NinjaScript input's bars series, please use the [Bars Series](bars.md) object (or the [BarsArray](barsarray.md) for multi-series input)

> **Note:** A ChartBars object will ONLY exist should the hosting NinjaScript type be loaded through a [Chart](chart.md).  For example, a Strategy would have access to a ChartBars property when running on a Chart, but would NOT when loaded through the [Strategies Grid](../addons/strategies_tab2.md) or [Strategy analyzer](../strategies/strategy_analyzer.md).

![ChartBars](../images/chartbars.png)
...
| [Properties](chartbars_properties.md) | Various ChartBar properties that have been configured from the Chart's [Data Series](../operations/working_with_price_data.md) menu. |
| [ToChartString()](chartbars_tochartstring.md).md) | A string formatted for the Chart's Data Series Label as well as the period. |
