...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\anchored-walk-forward-optimiza.htm) >>  **Navigation:**  [Operations](operations-1.htm) > [Strategy Analyzer](strategy_analyzer-1.htm) >  Anchored Walk Forward Optimization | [Previous page](walk_forward_optimize_a_strate-1.htm) [Return to chapter overview](strategy_analyzer-1.htm) [Next page](multi-objective_optimization-1.htm) |
...
|  |
| --- |
| Note: The [IncludeTradeHistoryInBacktest](includetradehistoryinbacktest-1.htm) property is set to false by default when a strategy is applied in the Strategy Analyzer for optimization. This provides for leaner memory usage, but at the expense of not being able to access Trade objects for historical trades. Thus, fields such as [SystemPerformance.AllTrades.Count](alltrades-1.htm) that rely on references to Trade objects will not have any such references to work with. If you would like to save these objects for reference in your code, you can set IncludeTradeHistoryInBacktest to true in the Configure state. For more information, see the [Working with Historical Trade Data](strategyanalyzer_properties_2-1.htm) page. |

![tog_minus](tog_minus-1.gif)        How to run an Anchored Walk Forward Optimization
