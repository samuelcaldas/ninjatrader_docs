...
|  |  |
| --- | --- |
| << [Click to Display Table of Contents](.\understanding_the_lifecycle_of.htm) >>  **Navigation:**  [NinjaScript](ninjascript-1.htm) > [Educational Resources](educational_resources-1.htm) >  NinjaScript Lifecycle | [Previous page](multi-time_frame__instruments-1.htm) [Return to chapter overview](educational_resources-1.htm) [Next page](using_3rd_party_indicators-1.htm) |

NinjaTrader uses a [State](state-1.htm) change system to represent various life cycles of your NinjaScript object.  For more basic indicators and strategies, simply understanding each State described on the [OnStateChange()](onstatechange-1.htm) page is sufficient.  However, for more advanced development projects, it is critical to understand how NinjaTrader calls these states for various instances throughout the lifetime of the entire application.
...
In both categories, [OnStateChange()](onstatechange-1.htm) is called at least twice:  once to State.SetDefaults acquiring various default property values, and then again to State.Terminated handling internal references cleanup.
...
2.The selected indicator is then [cloned](clone-1.htm) and SetDefaults is called again in order to display the default properties to the "Properties" grid.  This only occurs for the individual indicator.
...
•Resources should only be set up once an object has reached State.Configure or State.DataLoaded (see [best practices](ninjascript_best_practices-1.htm) for more information)
...
NinjaScript objects have a base clone method implemented which will iterating over all browsable properties and copy by value to the next instance. The rules follow the 'clone' rules described in the clone documentation located [here](clone-1.htm) and described above. The default behavior will work in almost all cases except for when you have some complex custom property which needs specific clone behavior. In which case we allow the ability to override Clone() and specify your own behavior.
...
By default basic types such as int, string, bool will all serialize without issue, if you have a complex property you want its setting maintained on restore then you need to create a string serialized representation of that class. The technique is shown in this example post [here](user_definable_color_inputs-1.htm) where we show how to serialize a color brush.